...
**`pkg/browser/cookies.go`**: `SessionCookie(url)` finds `cookies.sqlite` under `~/.floorp`, `~/.mozilla/firefox`, `~/.librewolf`, copies it (plus any `-wal`/`-shm`) to a temp dir to dodge the browser lock, and queries `moz_cookies` for the host's `session` cookie via the pure-Go `modernc.org/sqlite` driver. `main` calls it before `validateConfig` when `-from-browser` is set and no cookie was given. Chrome/Chromium is intentionally unsupported (OS-keychain-encrypted cookies).
