...
- File stats are committed **once**, in `commitFileStats`, only after a challenge fully succeeds — challenge-level retries re-run downloads, so counting mid-flight double-counts.
...
**`pkg/services/filesystem.go`** — writes to a temp file + atomic rename so a failed re-download never clobbers a prior good file; hashes via `io.MultiWriter` while writing (no re-read). Filenames are run through `utils.SanitizeName` and `..`/`.` are rejected to prevent path traversal.
